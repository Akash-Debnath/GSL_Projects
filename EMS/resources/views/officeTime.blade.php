                                    ...
                                    @if ($dc == $code)
                                        <tr data-widget="expandable-table" aria-expanded="true">
                                            ...
                                            <form action="{{ url('schecule-update', $emp->emp_id) }}" method="post">
                                                ...
                                                @endif
                                                <td> <button type="submit" onclick="return confirm('Change to {{ $emp->scheduled_attendance == 'N' ? 'Scheduled' : 'Non-scheduled' }}')" style="border: none; background-color:white; color:green;">{{ $emp->scheduled_attendance == "N" ? "Non-scheduled" : "Scheduled" }}</button> </td>
                                            </form>
                                            <form action="{{ url('roster-update', $emp->emp_id) }}" method="post" >
                                                ...
                                                @endif
                                                <td> <button type="submit" onclick="return confirm('Change to {{ $emp->roster == 'N' ? 'Roster' : 'Non-roster' }}')" style="border: none; background-color:white; color:green;">{{ $emp->roster == "N" ? "Non-roster" : "Roster" }}</button> </td>
                                            </form>
                                            <td>
                                                @php
                                                    $id = $emp->emp_id;
                           ...
                           <div class="row">
                               <div class="col-md-5">
                                   <div class="row">
                                        @foreach ($weekends as $key=>$value)
                                                <div class="col-12">
                                                    <input type="checkbox" {{ $wl->$key == 'Y' ? 'checked' : '' }} name="" id="">
                                                    <label for="sunday">{{ $value }}</label>      
                                                </div>
                                        @endforeach
                                    </div>                                                        
                               </div>
